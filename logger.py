...
class Logger:
    def __init__(self, agent: SACAgent, env_name: str, interval=1000, seed=0, **config):
        self.agent = agent
        self.log_dir = env_name[:-3] + "/" + datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
        self.start_time = 0
        self.duration = 0
    ...
    def _create_weights_folder(dir):
        Path("Checkpoints", dir).mkdir(exist_ok=True, parents=True)
    ...
    def log(self, episode, episode_reward, skill, logq_zs, step, *rng_states):
    ...
    def load_weights(self):
        model_dir = glob.glob("Checkpoints/" + self.env_name[:-3] + "/")
        model_dir.sort()
